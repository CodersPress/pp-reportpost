...
Description: This plugin will attach a form (jQuery / Ajax) just under your Listings Description that can be Invoked by a guest or registered user to make a report on posts / pages. If needed use shortcode [report]
Version: 2015.0819
...
add_action( 'init', 'rp_plugin_updater' );
function rp_plugin_updater() {
	if ( is_admin() ) { 
	include_once( dirname( __FILE__ ) . '/updater.php' );
		$config = array(
			'slug' => plugin_basename( __FILE__ ),
			'proper_folder_name' => 'pp-reportpost',
			'api_url' => 'https://api.github.com/repos/CodersPress/pp-reportpost',
			'raw_url' => 'https://raw.github.com/CodersPress/pp-reportpost/master',
			'github_url' => 'https://github.com/CodersPress/pp-reportpost',
			'zip_url' => 'https://github.com/CodersPress/pp-reportpost/zipball/master',
			'sslverify' => true,
			'access_token' => '82bc20b2c88393882c825186e6cb392c424fbff6',
		);
		new WP_RP_UPDATER( $config );
	}
}
...
add_shortcode( 'report', 'shorttag_func' );
...
function wprp_admin_menu()
{
	...
	add_menu_page('PP-REPORTPOST New Reports', 'Reports'.$newReports, 4, dirname(__FILE__)."/new-reports.php",'', WP_PLUGIN_URL.'/pp-reportpost/assets/reports.png');
...
function wprp_install()
{
	...
	if(!get_option('rp_thanks_msg'))
		update_option('rp_thanks_msg',"<strong>Thanks for Reporting [TITLE]</strong>");
...
{
	wp_enqueue_style('wprp-admin', WP_PLUGIN_URL."/pp-reportpost/assets/wprp-admin.css"); 
